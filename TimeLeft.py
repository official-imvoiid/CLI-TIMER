...
class CountdownApp:
    def __init__(self, root, direct=False, end_dt=None):
        self.root = root
        self.direct = direct
        self.timer_description = "Countdown Complete!"
        if direct:
    ...
    def build_ui(self):
        ...
        ttk.Label(self.start_tab, text="Timer Description (Max 100 Words):").pack(pady=(10, 0))
        self.desc_entry = ttk.Entry(self.start_tab, textvariable=self.desc_var, width=40)
    ...
    def apply_start(self):
        try:
            ...
            self.timer_description = self.desc_var.get().strip() or "Countdown Complete!"
    ...
    def _open_countdown(self, end_time):
        ...
        win = tk.Toplevel()
        win.title("Time Left")
        # Show description during countdown
        ttk.Label(win, text=self.timer_description, font=("Arial", 14), wraplength=400).pack(pady=(10, 0))
        lbl = ttk.Label(win, text="", font=(None, 24))
        lbl.pack(pady=10)
        pb = ttk.Progressbar(win, orient="horizontal", length=400, mode="determinate")
        pb.pack(pady=10)
        pb['maximum'] = total
    ...
    def _update_ct(self, win, lbl, pb, end_time, total):
        ...
        if rem <= 0:
            ...
            end_win.grab_set()
            # Show final message instead of description
            tk.Label(end_win, text="Countdown Ended", font=("Arial", 16)).pack(pady=(10, 5))
        ...
        m, s = divmod(rem_s, 60)
        lbl.config(text=f"{days}D {h:02d}:{m:02d}:{s:02d}")
        pb['value'] = total - rem
